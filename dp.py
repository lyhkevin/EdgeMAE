...
class Options():
    def __init__(self):
        ...
        self.parser.add_argument("--batch_size", default=120, type=int)
        ...
        self.parser.add_argument("--device_ids", default=[0,1,2])
        
        self.parser.add_argument('--img_save_path', type=str,default='./snapshot/dp/')
        self.parser.add_argument('--weight_save_path', type=str,default='./weight/dp/')
        ...
        self.parser.add_argument("--log_path", default='./log/dp.log')
...
mae = torch.nn.DataParallel(mae, device_ids=device_ids)
mae = mae.cuda(device=device_ids[0])
...
for epoch in range(0,opt.epoch):
    for i,img in enumerate(train_loader):
        ...
        img = img.cuda(device=device_ids[0])
        rec_loss, edge_loss,edge_gt,x_edge,x_rec,mask = mae(img,opt.masking_ratio,epoch)
        rec_loss, edge_loss = torch.mean(rec_loss), torch.mean(edge_loss)
